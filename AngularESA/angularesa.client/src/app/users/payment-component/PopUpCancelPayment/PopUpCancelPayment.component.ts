...
@Component({
  ...
  templateUrl: './PopUpCancelPayment.component.html',
  styleUrls: ['./PopUpCancelPayment.component.css']
})
export class PopUpCancelPaymentComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<PopUpCancelPaymentComponent>
  ) { }
  ...
  cancelPayment(selectedCard?: string): void {
    if (selectedCard) {
      let newCard: CardID = { cardID: selectedCard.toString() }
      this.isPaymentProcessing = true;
      //selectedCard.toString()
      if (newCard) {
        this.squareService.cancelPayment(newCard).subscribe(
          (response) => {

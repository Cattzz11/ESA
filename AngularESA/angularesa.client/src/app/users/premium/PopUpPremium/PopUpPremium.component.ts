...
@Component({
  selector: 'app-PopUpPremium',
  templateUrl: './PopUpPremium.component.html',
  styleUrls: ['./PopUpPremium.component.css']
})
export class PopUpPremiumComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<PopUpPremiumComponent>
  ) { }
  ...
  premiumSubscription(selectedCard?: string): void {
    if (selectedCard) {
      let newCard: CardID = { cardID: selectedCard.toString() }
      this.isPaymentProcessing = true;
      //selectedCard.toString()
      if (newCard) {
        this.squareService.premiumSubscription(newCard).subscribe(
          (response) => {
